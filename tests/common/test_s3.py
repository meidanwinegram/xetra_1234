...
class TestS3BucketConnectorMethods(unittest.TestCase):
    ...
    def test_write_df_to_s3_parquet(self):
        # Expected results
        return_exp = True
        data_frame_exp = pd.DataFrame([['A','B'],['C','D']], columns = ['col1', 'col2'])
        ...
        # Method execution
        with self.assertLogs() as logm:
            result = self.s3_bucket_conn.write_df_to_s3(data_frame_exp, key_exp, file_format)
            # Log test after method execution
            self.assertIn(log_exp, logm.output[0])
        # Test after method execution
        ...
        self.assertEqual(return_exp, result)
        self.assertTrue(df_result.equals(data_frame_exp))
        # Cleanup after test
        self.s3_bucket.delete_objects(
                        Delete={
                'Objects': [
    ...
    def test_write_to_df_wrong_format(self):
        """
        Tests the write_to_df for wrong_format
        if a supported format is given as argument
        """
        # Expected results
        df_exp = pd.DataFrame([['A','B'],['C','D']], columns = ['col1', 'col2'])
        key_exp='test.parquet'
        format_exp= 'wrong_format'
        log_exp = f'The file format {format_exp} is not supprted to be written to s3!'
